...
sub ageSeq {
	...
	###  fasta files
	opendir(ARGV,"$dat_dir");
	@files=grep(/\.fas$|\.fa$|\.fasta$|\.seq$/,readdir ARGV);
	...
	foreach my $file_blat_in (@files_fas){	
		...
		# Around line 174, update the command line construction:
		my $blat_bin = "/data/AGEseq/blat_exe/blat/blat"; # Or your actual path from 'which blat'
		if (! -e $blat_bin) { $blat_bin = "blat"; } # Fallback to system path
		my $command_line = "$blat_bin -tileSize=7 -oneOff=1 -maxGap=20 -minIdentity=70 -minScore=20 $file_blat_in $design_fas $blat_out";
...
# --- Add this at the very bottom of AGEseq.pl ---
# This allows the script to be run from the command line by Streamlit
if (!caller) {
    # Extract arguments from the command line (@ARGV)
    my ($dat_dir, $file_design, $final_out, $mismatch, $min, $wt, $indel) = @ARGV;
    
    # Call the existing function
    ageSeq($dat_dir, $file_design, $final_out, $mismatch, $min, $wt, $indel);
}
